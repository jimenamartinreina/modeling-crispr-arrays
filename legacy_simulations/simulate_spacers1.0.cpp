...
#define NUMREPS 1000		// Number of replicates
...
int main (int argc,char *argv[])
{
  if (argc<4)
   ...
   pEndemic = atof(argv[3]);
    ...
    {
        printf("Running simulation with MAXGEN = %d\n", Tmax);
        ...
        else
        {
            ...
            printf("Condition not met. Increasing MAXGEN to %d and repeating...\n", Tmax);
        }
    }
    ...
    printf("seed = %li\n", seed);
    printf("beta\tage\tefficacy\tfitness (means)\n");
    for (ii = 0; ii < Nsp; ii++)
        printf("%6.4f\t%6.4f\t%6.4f\t%6.4f\n", QuartilesBeta[ii][3], QuartilesAge[ii][3], QuartilesEff[ii][3], QuartilesFitness[ii][3]);
...
// Select spacer based on exp(-efficacy)
sumEff = 0;
ii = 0;
do
   {
